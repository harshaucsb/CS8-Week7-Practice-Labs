...
test_params = [
    ({}),
    ({'John': {'Midterm': 70, 'Final': 90}, 'Chelsea': {'Midterm': 78, 'Final': 85},  'Shawn': {'Midterm': 83, 'Final': 85}}),
    ({123: {'Midterm': 70, 'Final': 90}}),
]


max_score = len(test_params)

function_name = "calculate_class_midterm_average" # TODO

# TODO:  name the class according to the function name being tested
class CalculateClassMidtermAverage_Test(BaseClass):
    @partial_credit(max_score)
    @visibility('visible')
    def test1(self, set_score=None):
        """calculate_class_midterm_average(options, option)""" # TODO
